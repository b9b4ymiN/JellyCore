...
class OracleMCPServer {
  ...
  private chromaClient: ChromaHttpClient;
...
Philosophy: "Nothing is Deleted" — All interactions logged.`,
  ...
  /**
   * Private: Vector search using ChromaDB HTTP client
  ...
  private async vectorSearch(
    ...
    try {
      ...
      // Query via ChromaDB HTTP client
      const results = await this.chromaClient.query(query, limit, whereFilter);
  ...
  /**
   * Pre-connect to ChromaDB before MCP server starts
   */
...
/**
 * Main entry point
 * Pre-connect ChromaDB HTTP client before starting MCP server
 */
async function main() {
  ...
  // Pre-connect to ChromaDB
  try {
    console.error('[Startup] Connecting to ChromaDB...');
    await server.preConnectChroma();
    console.error('[Startup] ChromaDB connected successfully');
  } catch (e) {
