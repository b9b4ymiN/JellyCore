...
import { ANY_SLASH_COMMAND_REGEX } from './command-registry.js';
...
const THAI_COMPLEX_QUESTION = /หลักการ|อธิบาย|ต่างกัน|แตกต่าง|อันเดียวกัน|ยังไง|อย่างไร|ทำไม|เพราะอะไร|หมายถึง|คืออะไร|ควร|ได้ไหม|ไหม|มั้ย|หรือ/i;
...
export function classifyQuery(message: string): QueryClassification {
  ...
  const isThaiExplanatoryQuestion =
    THAI_SCRIPT.test(trimmed) && THAI_COMPLEX_QUESTION.test(trimmed);
  ...
  if (ANY_SLASH_COMMAND_REGEX.test(trimmed)) {
  ...
  if (isThaiExplanatoryQuestion) {
    return { tier: 'container-full', model: 'sonnet', reason: 'thai-explanation' };
  }
  if (len > 500) {
