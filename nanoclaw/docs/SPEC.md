...
Messages must start with the trigger pattern (default: `@Fon`):
- `@Fon what's the weather?` → ✅ Triggers Claude
- `@Fon help me` → ✅ Triggers (case insensitive)
- `Hey @Fon` → ❌ Ignored (trigger not at start)
...
[Jan 31 2:35 PM] John: @Fon what toppings do you recommend?
```
...
|---------|---------|--------|
| `@Assistant [message]` | `@Fon what's the weather?` | Talk to Claude |

### Commands Available in Main Channel Only

| Command | Example | Effect |
|---------|---------|--------|
| `@Assistant add group "Name"` | `@Fon add group "Family Chat"` | Register a new group |
| `@Assistant remove group "Name"` | `@Fon remove group "Work Team"` | Unregister a group |
| `@Assistant list groups` | `@Fon list groups` | Show registered groups |
| `@Assistant remember [fact]` | `@Fon remember I prefer dark mode` | Add to global memory |
...
```
User: @Fon remind me every Monday at 9am to review the weekly metrics
...
```
User: @Fon at 5pm today, send me a summary of today's emails
...
From any group:
- `@Fon list my scheduled tasks` - View tasks for this group
- `@Fon pause task [id]` - Pause a task
- `@Fon resume task [id]` - Resume a paused task
- `@Fon cancel task [id]` - Delete a task

From main channel:
- `@Fon list all tasks` - View tasks from all groups
- `@Fon schedule task for "Family Chat": [prompt]` - Schedule for another group
...
| "No groups registered" | Haven't added groups | Use `@Fon add group "Name"` in main |
